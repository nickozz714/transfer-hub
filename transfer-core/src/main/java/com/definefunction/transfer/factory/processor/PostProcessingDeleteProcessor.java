...
public class PostProcessingDeleteProcessor extends AbstractProcessorEvents implements Processor {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    public PostProcessingDeleteProcessor(InProgressService inProgressService, RecordService recordService, ProgressEventService progressEventService, URLBuilderService urlBuilderService, List<TransferLogging> transferLogging) {
    ...
    public void process(Exchange exchange) throws Exception {
        ...
        logger.info("Post-processing delete | " + exchange.getFromRouteId() + " | From: "+from+" | To: "+to+" | File: " + constructAbsoluteFileName(exchange));
            setPostProcessingBlobHeaders(exchange, ProcessingType.DELETE);
        }
    }
